...
type SortKey = 'title' | 'price' | 'duration' | 'destination'

export default function ToursManagePage() {
  const [tours, setTours] = useState<TourData[]>(TOURS)
  ...
  const handleDelete = async (id: string) => {
    ...
    // Replace with: await supabase.from('tours').delete().eq('id', id)
  ...
  const totalValue = tours.reduce((s, t) => s + t.price, 0)
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-7">
        <div>
          ...
          <p className="text-white/40 text-sm">{tours.length} packages · avg ${Math.round(totalValue / tours.length).toLocaleString()} per person</p>
        </div>
        <Link
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-7">
        {[
          ...
          { label: 'Avg Duration', value: `${Math.round(tours.reduce((s, t) => s + parseInt(t.duration), 0) / tours.length)} Days`, icon: '📅', color: '#60a5fa' },
        ].map(s => (
          <div
            key={s.label}
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex gap-1 p-1 rounded-xl" style={{ background: '#111', border: '1px solid rgba(255,255,255,0.06)' }}>
            {(['title', 'price', 'destination', 'duration'] as SortKey[]).map(s => (
              <button
                key={s}
                onClick={() => setSort(s)}
      ...
      {filtered.length === 0 ? (
        <div
          className="rounded-2xl border border-white/6 py-16 text-center"
          style={{ background: '#111' }}
        >
          <div className="text-4xl mb-3">🗺️</div>
          <p className="text-white/30 text-sm">No tours found</p>
          <Link href="/dashboard/add-tour" className="mt-4 inline-block text-xs font-semibold" style={{ color: '#C5A059' }}>
            + Add your first tour
          </Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {filtered.map(tour => (
            <div
              key={tour.id}
              className="group rounded-2xl border border-white/6 hover:border-yellow-700/30 transition-all duration-300 overflow-hidden"
              style={{ background: '#111' }}
            >
              {/* Image */}
              <div className="relative h-44 overflow-hidden">
                ...
                <div
                  ...
                  style={{ background: 'linear-gradient(to top, rgba(17,17,17,0.85) 0%, transparent 60%)' }}
                />
                {/* Live badge */}
                <div
                  className="absolute top-3 left-3 flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide"
              ...
              <div className="p-4">
                <div className="flex items-start justify-between gap-2 mb-1">
                  <p className="text-[10px] text-white/35 uppercase tracking-widest">{tour.destination}, {tour.country}</p>
                  <span
                ...
                <div className="flex flex-wrap gap-1 mb-3">
                  ...
                  {tour.highlights.length > 3 && (
                    <span className="text-[10px] text-white/25">+{tour.highlights.length - 3} more</span>
                  )}
                </div>
                ...
                <div className="flex items-center justify-between mb-4 pt-3 border-t border-white/6">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-white/60 text-sm font-semibold">{tour.groupSize}</p>
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  <Link
                    href={`/dashboard/add-tour?edit=${tour.id}`}
                  ...
                  <button
                    ...
                    className="px-3 py-2 rounded-xl text-xs font-semibold transition-all border border-white/8 text-white/25 hover:text-red-400 hover:border-red-500/30 hover:bg-red-500/8"
                  >
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      {confirmDelete && (
        ...
        >
          <div
            className="rounded-2xl p-7 w-full max-w-sm border border-white/10 text-center"
          ...
          >
            ...
            <p className="text-white/40 text-sm mb-6 leading-relaxed">
              <span className="text-white font-semibold">
                {tours.find(t => t.id === confirmDelete)?.title}
              </span>{' '}
              will be permanently removed from your site.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setConfirmDelete(null)}
                className="flex-1 py-2.5 rounded-xl text-sm font-semibold border border-white/10 text-white/50 hover:text-white transition-all"
              >
                Cancel
              </button>
              <button
